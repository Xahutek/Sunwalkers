...
[CreateAssetMenu(menuName = "Card/Lancer/Guardian")]
public class CardLancerGuardian : Card
{
    public float WagonShieldGainOnHit=5;
    ...
    public override void OnHit(GlobeEntity target, GlobeEntity attacker, float damage)
    {
        if (attacker != null && attacker is Wagon)
        {
            ...
            if (w.MasterCard == this)
                Caravan.main.GetLowestWagon().shieldHealth += WagonShieldGainOnHit + (1 * Level);
        }
    }
